...
class SEQGANs(nn.Module):
    ...
    def pretraining(self):
        ...
        for epoch in range(400):
            ...
            #输出loss和生成的字符
            self.vis.line(X=torch.tensor([epoch]), Y=torch.unsqueeze(torch.tensor(total_loss), 0), win='G_pre_loss', opts=dict(legend=['G_pre_loss']), update='append' if epoch > 0 else None)
            self.validate_G(epoch, 'pre_samples')
            time2 = time.time()
            print('epoch '+str(epoch)+': '+str(total_loss)+'   Times: '+str(time2-time1))
    def rollout(self):
    ...
    def backward_D(self, update=True, loss_f='LOG', is_epoch = False):#is_epoch: 是否遍历整个真实样本
        ...
        total_loss = total_loss/i
        return total_loss
    def validate_G(self, epoch, win):
        text_list = self.dataset.show(self.show_G())
        text = str(epoch) + ': '
        for b in range(self.batch_size):
            ...
            for word in text_list:
                if word[b] != '<R>':
                    ...
                    text += 'R' + ' '
            text += '</h5>'
        self.vis.text(text=text, win=win)
